...
onMounted(() => {
  nextTick(() => {
    try {
      console.log('音乐API地址:', import.meta.env.VITE_SONG_API);
      console.log('请求参数:', {server: props.songServer, type: props.songType, id: props.songId});
      console.log('完整请求URL:', `${import.meta.env.VITE_SONG_API}?server=${props.songServer}&type=${props.songType}&id=${props.songId}`);
      ...
      getPlayerList(props.songServer, props.songType, props.songId).then((res) => {
        ...
        playList.value = res;
        console.log("音乐加载完成");
        console.log('生成的播放列表:', playList.value);
