...
onMounted(() => {
  nextTick(() => {
    try {
      console.log("开始加载网易云音乐歌单");
      
      // 调用API获取网易云歌单数据
      getPlayerList(props.songServer, props.songType, props.songId)
        .then((data) => {
          if (!data || data.length === 0) {
            throw new Error("获取到的歌单数据为空");
          }
          
          // 生成歌单信息
          playIndex.value = Math.floor(Math.random() * data.length);
          playListCount.value = data.length;
          // 更改播放器加载状态
          ...
          console.log(
            ...
            "当前播放:",
            playIndex.value,
            playList.value[playIndex.value]?.title
          );
        })
        .catch((err) => {
          console.error("获取网易云歌单失败:", err);
